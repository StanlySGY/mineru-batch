...
# 3. 在上传页选择 easy-dataset 解析场景
# 该预设默认输出轻量 Markdown，并关闭图片、中间 JSON、模型输出等大体积产物

# 4. 拖拽 PDF 文件夹上传
# 系统会保留相对目录结构

# 5. 任务完成后，在任务列表勾选任务并点击“easy-dataset 包”
# 下载 Markdown-only ZIP 后导入 easy-dataset
```
...
- 批量重试、删除、转换、下载和 easy-dataset 包导出。
...
**Q: easy-dataset 包里为什么没有图片和 JSON？**

A: 这是预期行为。easy-dataset 包只导出 Markdown，用于降低导入体积；如需完整产物，请使用普通批量下载。
